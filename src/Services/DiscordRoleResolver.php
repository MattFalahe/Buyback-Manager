 ...
 * for the webhook role_mention picker.
 *
 * Ported verbatim from Structure Manager's DiscordRoleResolver per the
 * project rule that each plugin keeps its own resolver (rather than
 * consolidating into Manager Core) so standalone setup keeps working.
 *
...
class DiscordRoleResolver
{
    ...
    public static function listRoles(): array
    {
        ...
        foreach ($providers as $provider) {
            try {
                ...
                Log::warning('[Buyback Manager] DiscordRoleResolver: failed listing roles from ' . $provider . ': ' . $e->getMessage());
                continue;
            }
    ...
    /**
     * Describe a stored role-mention value for display.
     */
